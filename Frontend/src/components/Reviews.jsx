...
export default function Reviews({ productId, checkOrderHistory }) {
  ...
  const rows = useMemo(
    () =>
      [5, 4, 3, 2, 1].map((star) => ({
        ...
        pct: stats.total
          ? Math.round(((stats.distribution?.[star] || 0) / stats.total) * 100)
          : 0,
      })),
    [stats]
  );
  ...
  async function handleVote(id, value) {
    try {
      ...
      const updated = await ReviewService.list(productId, {
        page,
        limit,
        sort,
      });
  ...
  async function submitReview() {
    ...
    try {
      ...
      toast.success(
        myReview ? "Đã cập nhật đánh giá của bạn" : "Gửi đánh giá thành công"
      );
    } catch (e) {
                    ...
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        ...
                        className={`text-xl leading-none ${
                          s <= newRating ? "text-yellow-400" : "text-gray-300"
                        }`}
                ...
                <div className="mt-3">
                  ...
                  {myReview && (
                    <span className="ml-3 text-xs text-gray-500">
                      Bạn đã đánh giá {myReview.rating}/5 vào{" "}
    ...
    {SORTS.map((s) => (
      ...
      >
        ...
        <div className="mt-4 divide-y rounded-xl border bg-white">
          {loading && (
            <div className="p-6 text-sm text-gray-500">Đang tải đánh giá…</div>
          )}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 hover:text-emerald-700"
                    >
                      <ThumbsDown className="h-4 w-4" /> Không hữu ích (
                      {rv.downVotes || 0})
                    </button>
                  </div>
                </div>
