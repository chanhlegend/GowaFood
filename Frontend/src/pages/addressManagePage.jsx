...
const AddressManagement = () => {
  ...
  const [loading, setLoading] = useState(true); // loading tổng cho lần load đầu
  ...
  const [selectedWard, setSelectedWard] = useState(null); // {code, name}
  const [isWardDropdownOpen, setIsWardDropdownOpen] = useState(false);
  const wardWrapperRef = useRef(null);

  // Hàm bỏ dấu + chuẩn hóa text để search tiếng Việt
  ...
  // Danh sách communes đã lọc theo wardInput
  const filteredCommunes = communes.filter((commune) =>
    normalizeText(commune.name).includes(normalizeText(wardInput))
  );

  // Lấy danh sách communes từ API
  ...
  // Khi mở form edit, tự set selectedWard + wardInput theo địa chỉ cũ
  useEffect(() => {
    if (editingAddress && communes.length > 0) {
      const found = communes.find(
        (c) =>
          c.code === editingAddress.wardCode || c.name === editingAddress.ward
      );
      if (found) {
        setSelectedWard({ code: found.code, name: found.name });
        setWardInput(found.name);
      } else {
        setSelectedWard(null);
        setWardInput(editingAddress.ward || "");
      }
    } else if (!editingAddress) {
      setSelectedWard(null);
      setWardInput("");
    }
  }, [editingAddress, communes]);

  // Click outside để đóng dropdown xã/phường
  useEffect(() => {
  ...
  // Lấy tọa độ từ địa chỉ: chỉ chấp nhận nếu Nominatim trả về, nếu không => null
  ...
  // Submit form: thêm / cập nhật địa chỉ + check khoảng cách
  const handleSubmitForm = async (formData) => {
    try {
      ...
      // Validate địa chỉ chi tiết: không cho kiểu "213" quá ngắn, thiếu tên đường
      const addressDetail = formData.address.trim();
      if (addressDetail.length < 6 || !addressDetail.includes(" ")) {
        setError(
          "Vui lòng nhập rõ số nhà và tên đường (ví dụ: 213 Lý Thường Kiệt)"
        );
        return;
      }
      ...
      if (!distance || distance.success === false) {
        setMess(
          "Không tính được khoảng cách đến cửa hàng, vui lòng kiểm tra lại địa chỉ."
        );
        setAlertMess(true);
        return;
      }
      ...
      const addressData = {
        ...
        distanceToStore: distance.distance,
            ...
            <div className="max-w-4xl">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 gap-4">
                ...
                <button
                  ...
                  className="cursor-pointer px-4 py-2 bg-white border-2 border-green-500 text-green-600 rounded-lg hover:bg-green-50 transition-colors font-medium text-sm sm:text-base w-full sm:w-auto"
                >
                  + Thêm địa chỉ
                </button>
              </div>
                ...
                <div className="mt-6 sm:mt-8 bg-white rounded-lg border border-gray-200 p-4 sm:p-6 shadow-sm">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    ...
                    <button
                      ...
                      className="cursor-pointer text-gray-400 hover:text-gray-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
                    >
                      <svg
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <input
                          ...
                          required
                          disabled={submittingAddress}
                          placeholder="Ví dụ: 213 Lý Thường Kiệt"
                          className="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
                        />
                      </div>

                      {/* Combobox Xã/Phường */}
                      <div ref={wardWrapperRef} className="relative">
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Xã/Phường
                        </label>
                        <input
                          ...
                          onFocus={() => {
                            if (!loadingCommunes) setIsWardDropdownOpen(true);
                          }}
                          disabled={loadingCommunes || submittingAddress}
                          className="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
                        />
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                      <button
                        ...
                        className="cursor-pointer px-6 py-2 text-sm sm:text-base text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors min-h-[48px] order-2 sm:order-1 disabled:opacity-60 disabled:cursor-not-allowed"
                        disabled={submittingAddress}
                      >
                        Hủy
                      </button>
                      <button
                        type="submit"
                        disabled={submittingAddress}
                        className="cursor-pointer px-6 py-2 text-sm sm:text-base bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors min-h-[48px] order-1 sm:order-2 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        {submittingAddress || loadingCoordinates
                          ? "Đang lưu..."
                          : editingAddress
                          ? "Cập nhật địa chỉ"
                          : "Thêm địa chỉ"}
                ...
                {addresses.length === 0 ? (
                  ...
                  addresses.map((address) => {
                    const isSettingDefault =
                      settingDefaultId === address._id && !address.isDefault;
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            ...
                            {!address.isDefault && (
                              <button
                                ...
                                className="cursor-pointer px-2 sm:px-3 py-1 bg-green-100 text-green-700 text-xs sm:text-sm rounded-full hover:bg-green-200 transition-colors w-fit disabled:opacity-60 disabled:cursor-not-allowed"
                              >
                                {isSettingDefault
                                  ? "Đang đặt..."
                                  : "Đặt làm địa chỉ mặc định"}
                          ...
                          <div className="flex items-center space-x-2">
                            <button
                              ...
                              className="cursor-pointer p-2 text-gray-400 hover:text-blue-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
                              title="Chỉnh sửa"
                            >
                              <svg
                            ...
                            <button
                              ...
                              className="cursor-pointer p-2 text-gray-400 hover:text-red-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
                              title="Xóa"
                            >
                              <svg
                                className="w-4 h-4 sm:w-5 sm:h-5"
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="cursor-pointer text-gray-400 hover:text-gray-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                ...
                <button
                  ...
                  disabled={deleteLoading}
                  className="cursor-pointer px-6 py-2 text-sm sm:text-base bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors min-h-[48px] order-1 sm:order-2 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {deleteLoading ? "Đang xóa..." : "Xóa địa chỉ"}
                </button>
              </div>
            </div>
          </div>
